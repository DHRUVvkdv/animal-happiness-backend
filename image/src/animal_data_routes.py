...
DYNAMODB_ANIMAL_TABLE = os.getenv("DYNAMODB_TABLE_NAME_ANIMAL", "animal-happiness-data")
...
# Create router
router = APIRouter(tags=["AnimalHappiness"])
...
# Utility function to create the DynamoDB table if it doesn't exist
def create_animal_table_if_not_exists():
    ...
    try:
        ...
        logger.info(f"Created table {DYNAMODB_ANIMAL_TABLE}: {response}")
    except Exception as e:
        logger.error(f"Error creating DynamoDB table: {e}")


# Create the table when the application starts (for local development)
if os.getenv("IS_USING_IMAGE_RUNTIME") and __name__ != "__main__":
